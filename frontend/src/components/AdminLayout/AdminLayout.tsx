...
const AdminLayout: React.FC = () => {
  ...
  const getDesktopNavStyle = ({ isActive }: { isActive: boolean }) =>
    `flex items-center gap-4 px-5 py-3.5 rounded-xl text-base transition-all duration-300 border-l-4 ${
      isActive
  ...
  const getMobileNavStyle = ({ isActive }: { isActive: boolean }) =>
    `flex flex-col items-center justify-center rounded-xl w-16 py-2 transition-all duration-300 ease-in-out ${
      isActive
  ...
  return (
    <div className="bg-background text-on-background min-h-screen flex flex-col md:flex-row pb-20 md:pb-0 antialiased">
      ...
      {/* Main Content Canvas */}
      <main className="flex-1 md:ml-[280px] p-6 md:p-8 md:p-margin-page bg-background">
        <Outlet />
      </main>
